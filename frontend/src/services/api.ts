...
// Initialize token from localStorage on module load (for page refreshes)
try {
...
export async function signup(
  ...
  username: string,
  password: string
): Promise<{ message: string; user: User; verificationCode?: string }> {
  const { data } = await api.post("/auth/signup", { email, username, password });
  return {
    message: data.message as string,
    user: data.user as User,
    verificationCode: data.verification_code as string | undefined,
  };
}
...
export async function forgotPassword(username: string, email: string): Promise<{ resetCode?: string }> {
  ...
  return { resetCode: data.reset_code as string | undefined };
}
